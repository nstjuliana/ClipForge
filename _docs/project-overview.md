...
You'll build a desktop video editor using **Electron.js** that handles screen recording, webcam capture, clip import, timeline editing, and video export.
...
* Desktop framework (Electron.js with React/Vue frontend)  
* Media processing library (FFmpeg via fluent-ffmpeg or @ffmpeg/ffmpeg)  
...
**For Electron:** Use Electron's `desktopCapturer` API (available in renderer process) to list available screens/windows. This returns sources that can be passed to `getUserMedia()` for screen capture. For webcam, use standard `navigator.mediaDevices.getUserMedia()`. Use `getDisplayMedia()` for screen sharing with user selection.

**Best approach:** Use `desktopCapturer.getSources()` to list screens, then use the source with `getUserMedia()` for high-quality screen recording.
...
**Desktop Framework:** Electron.js (Node.js backend with Chromium for cross-platform desktop apps)

**Frontend:** React, Vue, Svelte, or Vanilla JS

**Media Processing:** FFmpeg (fluent-ffmpeg for Node.js backend, or @ffmpeg/ffmpeg for browser context)
